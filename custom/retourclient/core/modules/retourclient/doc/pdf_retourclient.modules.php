...
/**
 *  \file       core/modules/retourclient/doc/pdf_retourclient.modules.php
 *  \ingroup    retourclient
 *  \brief      File of class to generate document from retourclient template
 */

dol_include_once('/retourclient/core/modules/retourclient/modules_retourclient.php');
 ...
 */
class pdf_retourclient extends ModelePDFRetourClient
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->dir_output) {
			...
			if ($object->specimen) {
				$dir = $conf->retourclient->dir_output.'/';
				$file = $dir."/SPECIMEN.pdf";
			} else {
				$objectref = dol_sanitizeFileName('RetourClient_'.$object->id);
				$dir = $conf->retourclient->dir_output.'/'.$object->id;
			...
			if (file_exists($dir)) {
				...
				$pdf->SetTitle($outputlangs->convToOutputCharset("Retour client"));
				$pdf->SetSubject("Retour client");
				$pdf->SetCreator("Dolibarr ".DOL_VERSION);
				$pdf->SetAuthor($outputlangs->convToOutputCharset($user->getFullName($outputlangs)));
				$pdf->SetKeyWords($outputlangs->convToOutputCharset("Retour client"));
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					$pdf->MultiCell($this->posxqty - $this->posxup - 0.8, 3, price($object->lines[$i]['montant_ht']), 0, 'R', 0);
					...
					$pdf->MultiCell($this->posxdiscount - $this->posxtva - 0.8, 4, price($object->lines[$i]['taux_tva']).'%', 0, 'R');


					// Total HT line
					$pdf->SetXY($this->postotalht, $curY);
					$pdf->MultiCell($this->page_largeur - $this->marge_droite - $this->postotalht, 3, price($object->lines[$i]['total_ht']), 0, 'R', 0);
	...
	protected function _tableau_tot(&$pdf, $object, $posy, $outputlangs, $outputlangsbis)
	{
		...
		$pdf->MultiCell($largcol2, $tab2_hl, price(price2num($object->montant_ht, 'MT'), 0, $outputlangs), 0, 'R', 1);
		...
		$pdf->MultiCell($largcol2, $tab2_hl, price(price2num($object->montant_tva, 'MT'), 0, $outputlangs), 0, 'R', 1);
		...
		$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
		$pdf->MultiCell($largcol2, $tab2_hl, price(price2num($object->montant_ttc, 'MT'), 0, $outputlangs), $useborder, 'R', 1);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		if ($showaddress) {
			...
			$sql = "SELECT f.fk_soc
					...
					INNER JOIN ".MAIN_DB_PREFIX."facture AS f ON (r.fk_facture = f.rowid)
					WHERE r.rowid = ".$object->id;
	
			$resql = $db->query($sql);
			$obj = $db->fetch_object($resql);
			$_client = new Societe($db);
			$_client->fetch($obj->fk_soc);

			$carac_client = pdf_build_address($outputlangs, $_client, $_client, '', $usecontact, 'target', $object);
			// Show recipient
			$widthrecbox = 100;
			...
			$pdf->SetXY($posx + 2, $posy + 3);
			$pdf->SetFont('', 'B', $default_font_size);
			$pdf->MultiCell($widthrecbox - 2, 2, $_client->nom, 0, $ltrdirection);
