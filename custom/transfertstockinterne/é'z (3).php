...
if (! empty($conf->transfertstockinterne->enabled) && $user->rights->transfertstockinterne->transfert_stock->create)
{
	...
	if (empty($user->fk_warehouse)){
		...
		if ($_num > 0){
			...
			$sql = "SELECT p.ref, p.label, s.fk_product, s.reel AS stock_depot, IFNULL(ABS(SUM(m.value)),0) AS qte_vendue, ifnull(ps.reel, 0) AS stock_magasin, , ef.uniteachat as options_uniteachat
