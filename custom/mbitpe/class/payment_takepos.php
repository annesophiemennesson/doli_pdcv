...
function status($user, $db, $conf, $object, $langs)
{
	...
	$totalttc = 20;																					// TOTAL TTC
	$dejaregle = 0;				// DEJA REGLE
	...
	if ($result !== "" && $result !== false) {
		...
		if ($status == "10") {                                                                                                		// SI OPERATION EFFECTUE
			...
			$db->begin();
			$db->query("INSERT INTO " . MAIN_DB_PREFIX . "mbi_tpe (entity, object, primary_account_number, private_data) VALUES ('" . $conf->entity . "', '" . $object->id . "', '" . $primary_account_number . "', '" . $private_data . "')");
			$db->commit();
		} else {
			echo "0";
		}
...
$terminalIP = "MBI_TERMINAL_IP" . $_POST['terminal'];
$terminalPort = "MBI_TERMINAL_PORT" . $_POST['terminal'];
$host = $conf->global->$terminalIP;

if (!empty($host) && $host !== "" && $host !== NULL) {
