...
class Transfert_stock extends CommonObject
{
	...
	//  */
	// public $table_element_line = 'transfertstockinterne_transfert_stockline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_transfert_stock';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Transfert_stockline';
	...
	//  */
	// protected $childtablesoncascade = array('transfertstockinterne_transfert_stockdet');

	// /**
	//  * @var Transfert_stockLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->transfertstockinterne->transfert_stock->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->transfertstockinterne->transfert_stock->transfert_stock_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('TRANSFERT_STOCK_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'transfert_stock/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'transfert_stock/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->transfertstockinterne->dir_output.'/transfert_stock/'.$oldref;
				$dirdest = $conf->transfertstockinterne->dir_output.'/transfert_stock/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->transfertstockinterne->dir_output.'/transfert_stock/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'TRANSFERT_STOCK_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'TRANSFERT_STOCK_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'TRANSFERT_STOCK_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Transfert_stock").'</u>';
		...
		$url = dol_buildpath('/transfertstockinterne/transfert_stock_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans("ShowTransfert_stock");
		...
		$hookmanager->initHooks(array('transfert_stockdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new Transfert_stockLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_transfert_stock = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->TRANSFERTSTOCKINTERNE_TRANSFERT_STOCK_ADDON)) {
			$conf->global->TRANSFERTSTOCKINTERNE_TRANSFERT_STOCK_ADDON = 'mod_transfert_stock_standard';
		}

		if (!empty($conf->global->TRANSFERTSTOCKINTERNE_TRANSFERT_STOCK_ADDON)) {
			$mybool = false;

			$file = $conf->global->TRANSFERTSTOCKINTERNE_TRANSFERT_STOCK_ADDON.".php";
			$classname = $conf->global->TRANSFERTSTOCKINTERNE_TRANSFERT_STOCK_ADDON;
	...
	public function loadProduits()
	{
		...
		$sql = "SELECT p.label, tp.qte_prepa as qte_prod, tl.qte_prepa as qte_lot, batch, eatby
