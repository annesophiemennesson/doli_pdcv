...
if (! empty($conf->transfertstockinterne->enabled) && $user->rights->transfertstockinterne->transfert_stock->valid)
{
	...
	$today = new DateTime();
	$sql = "SELECT p.label, s.fk_product, s.reel AS stock, qte_demande, qte_valide, tp.rowid, commentaire_demande, commentaire_valide, t.fk_entrepot_depart
	...
	if ($resql)
	{
		...
		if ($num > 0)
		{
			...
			while ($i < $num)
			{
				...
				if ($num2 > 0){
					print '<tr id="more_'.$obj->fk_product.'" class="'.($obj->qte_valide > 0 ? 'hidden' : '').'"><td colspan="6">';
					print '<table class="noborder centpercent">';
					print '<tr class="liste_titre">';
