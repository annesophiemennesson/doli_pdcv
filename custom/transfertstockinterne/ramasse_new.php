...
if (! empty($conf->transfertstockinterne->enabled) && $user->rights->transfertstockinterne->transfert_stock->create_ramasse)
{
	...
	if (empty($user->fk_warehouse)){
		...
		$sql = "SELECT p.ref, p.label, s.fk_product, s.reel AS stock, IFNULL(ABS(SUM(m.value)),0) AS qte_vendue, ef.uniteachat
		...
		if ($resql)
		{
			...
			if ($num > 0)
			{
				...
				while ($i < $num)
				{
					...
					if ($num2 > 0){
						print '<tr class="more_'.$obj->fk_product.'" id="more_'.$obj->fk_product.'"><td colspan="5">';
						print '<table class="noborder centpercent">';
						print '<tr class="liste_titre">';
