...
$langs->loadLangs(array("main", "bills", "cashdesk", "banks", "mbitpe@mbitpe"));
...
?>
	...
	function Validate(payment)
	{
		...
		if (payment === 'card' || payment === 'CB') {
			if (amountpayed == 0) {
				amountpayed = <?php echo $remaintopay ?>;
			}
			var post = undefined;
			post = $.post( '<?php echo dol_buildpath("/mbitpe/class/payment_takepos.php", 1) ?>', { token: '<?php echo newToken() ?>', terminal: <?php echo $_SESSION["takeposterminal"] ?>, montantTerminal: amountpayed, object: invoiceid },
				function(data) {
					console.log(data);
					if (data == '1') {
						$('<span><?php echo $langs->trans('MBITPEOperationPerformed') ?></span><br><br><i style=\"color: green; margin-bottom: 20px;\" class=\"fas fa-3x fa-check-circle\"></i><br>').appendTo('#status');
						...
						$('<span><?php echo $langs->trans('MBITPEOperationNotPerformed') ?></span><br><br><i style=\"color: red; margin-bottom: 20px;\" class=\"fas fa-3x fa-exclamation-circle\"></i><br>').appendTo('#status');
					} else if (data == '-1') {
						if (amountpayed > <?php echo $invoice->total_ttc; ?>) {
							amountpayed = <?php echo $invoice->total_ttc; ?>;
						}
