...
llxHeader("", "Liste des transferts à réceptionner");

print load_fiche_titre("Liste des transferts à réceptionner", '', '');

print '<div class="fichecenter">';
if ($message == 'fin'){
	print '<p class="message">Transfert réceptionné !</p>';
}
...
if (! empty($conf->transfertstockinterne->enabled) && $user->rights->transfertstockinterne->transfert_stock->reception)
{
	...
	if (empty($user->fk_warehouse)){
        ...
        $sql = "SELECT e.ref as depart, e2.ref as arrivee, date_creation, t.label, COUNT(p.rowid) AS nb_produit, SUM(p.qte_prepa) AS total_qte, t.rowid
                ...
                WHERE date_reception IS NULL AND date_prepa IS NOT NULL AND fk_entrepot_arrivee = ".$user->fk_warehouse."
                GROUP BY t.rowid;";
        ...
        if ($resql)
        {
            ...
            if ($num > 0)
            {
                ...
                while ($i < $num)
                {
                    ...
                    print '<td><a class="button" href="'.dol_buildpath('/custom/transfertstockinterne/reception.php?id='.$obj->rowid, 1).'">Réceptionner</a></td>';
                    print '</tr>';
                    $i++;
                }
            }
            else
            {
                ...
                print '<tr class="oddeven"><td colspan="7" class="center">Aucun transfert à réceptionner</td></tr>';
            }
            print "</form></table><br>";
